...
struct ColumnInfo { name: String, data_type: String, }
#[derive(Serialize, Clone, Debug)]
...
async fn sync_schema(connection: Connection, db_name: String, conn_state: State<'_, DbConnection>) -> Result<(), String> {
    ...
    db_conn.execute("DELETE FROM cached_columns WHERE table_id IN (SELECT id FROM cached_tables WHERE connection_name = ?1 AND db_name = ?2)", params![&connection.name, &db_name]).map_err(|e| e.to_string())?;
    db_conn.execute("DELETE FROM cached_tables WHERE connection_name = ?1 AND db_name = ?2", params![&connection.name, &db_name]).map_err(|e| e.to_string())?;
    ...
    for row in rows {
        ...
        // Chave única para o mapa: schema.tabela
        let qualified_table_name = format!("{}.{}", schema_name, table_name);

        let table_id = match table_map.get(&qualified_table_name) {
            Some(id) => *id,
            None => {
                // ALTERAÇÃO: Salva o schema_name no cache
                db_conn.execute("INSERT INTO cached_tables (connection_name, db_name, schema_name, table_name) VALUES (?1, ?2, ?3, ?4)", params![&connection.name, &db_name, &schema_name, &table_name]).map_err(|e| e.to_string())?;
                let id = db_conn.last_insert_rowid();
                table_map.insert(qualified_table_name.clone(), id);
                id
            }
        };
        db_conn.execute("INSERT INTO cached_columns (table_id, column_name, data_type) VALUES (?1, ?2, ?3)", params![table_id, &column_name, &data_type]).map_err(|e| e.to_string())?;
    }
    Ok(())
}

#[tauri::command]
fn get_indexed_databases(connection_name: String, conn_state: State<DbConnection>) -> Result<Vec<String>, String> {
    let db_conn_mutex = conn_state.0.lock().map_err(|e| e.to_string())?;
    let db_conn = db_conn_mutex.as_ref().ok_or("SQLite connection not initialized")?;
    let mut stmt = db_conn.prepare("SELECT DISTINCT db_name FROM cached_tables WHERE connection_name = ?1").map_err(|e| e.to_string())?;
    let db_iter = stmt.query_map(params![&connection_name], |row| row.get(0)).map_err(|e| e.to_string())?;
    let mut indexed_dbs = Vec::new();
    for db_name in db_iter { indexed_dbs.push(db_name.map_err(|e| e.to_string())?); }
    Ok(indexed_dbs)
}

#[tauri::command]
fn get_cached_schema(connection_name: String, db_name: String, conn_state: State<DbConnection>) -> Result<SchemaInfo, String> {
    let db_conn_mutex = conn_state.0.lock().map_err(|e| e.to_string())?;
    let db_conn = db_conn_mutex.as_ref().ok_or("SQLite connection not initialized")?;

    // ALTERAÇÃO: Busca o schema_name junto com o table_name
    let mut stmt = db_conn.prepare("SELECT id, schema_name, table_name FROM cached_tables WHERE connection_name = ?1 AND db_name = ?2 ORDER BY schema_name, table_name")
        .map_err(|e| e.to_string())?;
    let mut table_rows = stmt.query(params![&connection_name, &db_name]).map_err(|e| e.to_string())?;

    let mut tables = Vec::new();
    while let Some(row) = table_rows.next().map_err(|e| e.to_string())? {
        let table_id: i64 = row.get(0).map_err(|e| e.to_string())?;
        let schema_name: String = row.get(1).map_err(|e| e.to_string())?;
        let table_name: String = row.get(2).map_err(|e| e.to_string())?;

        let mut col_stmt = db_conn.prepare("SELECT column_name, data_type FROM cached_columns WHERE table_id = ?1 ORDER BY column_name").map_err(|e| e.to_string())?;
        let mut col_rows = col_stmt.query(params![&table_id]).map_err(|e| e.to_string())?;
        
        let mut columns = Vec::new();
        while let Some(col_row) = col_rows.next().map_err(|e| e.to_string())? {
            columns.push(ColumnInfo {
                name: col_row.get(0).map_err(|e| e.to_string())?,
                data_type: col_row.get(1).map_err(|e| e.to_string())?,
            });
        }
        tables.push(TableInfo { schema: schema_name, name: table_name, columns });
    }
